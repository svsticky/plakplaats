...
function createCard(id, imagesrc) {
  console.log("HALLO CARD CREATEO!");
  ...
  function moveCard(deltaX) {
    if (
      objectX + deltaX > -1 * maxRotationIn &&
      objectX + deltaX < maxRotationIn
    ) {
    ...
    if (id == null) {
      ...
      logoAddRequest.open(
        ...
        "/addLogo?&name=" +
          logoEditName.value +
          "&color=" +
          encodeURIComponent(logoEditColor.value)
      );
